...
def add_address_view(request):
    ...
    if request.method != "POST":
        session_lat = request.session.get("customer_lat")
        session_lon = request.session.get("customer_lon")
        if session_lat not in (None, "") and session_lon not in (None, ""):
            initial_location = {
...
def checkout_view(request):
    ...
    # Actual AMEXA order is created only after payment verification.
    if payment_method != "COD":
        request.session["amexa_pending_payment"] = {
            "address_id": address_id,
            "payment_method": payment_method,
            "use_coins": request.POST.get("use_coins", "0") or "0",
        }
        request.session.modified = True
        return redirect("razorpay_start")
...
def location_picker_view(request):
    ...
    saved_address_text = (
        request.session.get("customer_location_address", "")
        or ""
    ).strip()
...
def shopkeeper_orders_view(request):
    ...
    # Pending orders must remain Pending until the shopkeeper taps Accept.
    # Do not auto-confirm while opening the Orders page.
...
def shopkeeper_order_accept_view(request, order_number):
    ...
    try:
        with transaction.atomic():
            ...
            task, created = OrderPickingTask.objects.get_or_create(
                order=order,
                defaults={
                    "shop": profile.shop,
                    "status": "WAITING",
            ...
            for order_item in order.items.all():
                picking_item, item_created = OrderPickingItem.objects.get_or_create(
                    order_item=order_item,
                    defaults={
                        "task": task,
                ...
                if picking_item.task_id != task.id:
                    picking_item.task = task

                picking_item.required_quantity = order_item.quantity

                if picking_item.picked_quantity > order_item.quantity:
                    picking_item.picked_quantity = order_item.quantity
        ...
        messages.error(
            request,
            f"ACCEPT ERROR: {type(error).__name__}: {error}",
        )
...
def shopkeeper_order_reject_view(request, order_number):
    ...
    order = get_object_or_404(
        Order.objects.select_related(
            "user", "address", "shop", "master_order"
        ),
        order_number=order_number,
        shop=profile.shop,
    )
    ...
    if request.method == "POST":
        ...
        try:
            with transaction.atomic():
                ...
                task = OrderPickingTask.objects.filter(
                    order=locked_order
                ).first()

                if (
                    task
                    and task.status not in {"WAITING", "CANCELLED"}
                ):
                    messages.error(
                        request,
            ...
            messages.error(
                request,
                f"REJECT ERROR: {type(error).__name__}: {error}",
            )
        ...
        messages.success(
            request,
            f"Order #{order.order_number} successfully rejected.",
        )
        return redirect(
            "shopkeeper_order_reject",
            order_number=order.order_number,
...
@login_required
def shopkeeper_pick_scan_view(request, order_number):
    """
    Shopkeeper UPC-based picking endpoint.
    One successful UPC scan = exactly one unit.
    It never allows picked_quantity to exceed required_quantity.
    """
    profile = _shopkeeper_app_profile(request)
    if profile is None:
        return redirect("shopkeeper_dashboard")
    ...
    if request.method != "POST":
        return redirect("shopkeeper_pick_order", order_number=order_number)
    ...
    with transaction.atomic():
        ...
        if task.order.status == "Cancelled" or task.status == "CANCELLED":
        ...
        if not barcode:
            messages.error(request, "UPC / barcode scan required.")
            return redirect("shopkeeper_pick_order", order_number=order_number)
        ...
        for item in picking_items:
            ...
            allowed_barcodes = {
                (row.barcode or "").strip().upper()
                for row in product.barcodes.all()
                if (row.barcode or "").strip()
            }
        ...
        if matched_item is None:
            messages.error(
                request,
                "Wrong product. This UPC does not belong to this order.",
            )
            return redirect("shopkeeper_pick_order", order_number=order_number)

        if matched_item.picked_quantity >= matched_item.required_quantity:
            messages.error(
                request,
                "Required quantity already complete. Extra unit is NOT allowed.",
            )
            return redirect("shopkeeper_pick_order", order_number=order_number)

        # Exactly one unit per successful scan.
        matched_item.picked_quantity += 1
        matched_item.last_scanned_barcode = barcode
        matched_item.save(
            update_fields=[
                "picked_quantity",
        ...
        if task.status in {"WAITING", "ACCEPTED"}:
            task.status = "PICKING"
            task.save(update_fields=["status", "updated_at"])
        ...
        product_name = (
            matched_item.order_item.product_name
            or (
                matched_item.order_item.product.name
                if matched_item.order_item.product
                else "Product"
            )
        )
        ...
        messages.success(
            ...
            (
                f"{product_name}: "
                f"{matched_item.picked_quantity}/"
                f"{matched_item.required_quantity} picked."
            ),
        )
...
@login_required
def shopkeeper_complete_picking_view(request, order_number):
    """
    Completes picking only when every ordered item's picked quantity
    exactly equals its required quantity.
    """
    profile = _shopkeeper_app_profile(request)
    if profile is None:
        return redirect("shopkeeper_dashboard")
    ...
    with transaction.atomic():
        ...
        if task.order.status == "Cancelled" or task.status == "CANCELLED":
            messages.error(request, "Cancelled order cannot be packed.")
            return redirect("shopkeeper_orders")
        ...
        if not items:
            messages.error(request, "No picking items found for this order.")
            return redirect("shopkeeper_pick_order", order_number=order_number)
        ...
        if incomplete:
            remaining_units = sum(
                max(0, item.required_quantity - item.picked_quantity)
                for item in incomplete
            )
            messages.error(
                request,
                f"Picking incomplete. {remaining_units} unit(s) still remaining.",
            )
            return redirect("shopkeeper_pick_order", order_number=order_number)

        # Exact quantities verified.
        task.status = "PACKED"
        task.packed_at = timezone.now()
        task.save(update_fields=["status", "packed_at", "updated_at"])

        if task.order.status in {"Confirmed", "Preparing"}:
            task.order.status = "Preparing"
            task.order.save(update_fields=["status", "updated_at"])

    # Rider assignment is non-critical: picking must stay completed even
    # if no rider is currently available.
    try:
    ...
    messages.success(
        request,
        "Picking completed. Parcel is packed. Rider details opened.",
    )
    return redirect(
        "shopkeeper_rider_detail",
        order_number=order_number,
...
@login_required
def shopkeeper_rider_detail_view(request, order_number):
    """
    Shopkeeper handover screen after packing.
    Shows assigned rider identity, phone, vehicle and latest assignment GPS.
    """
    profile = _shopkeeper_app_profile(request)
    if profile is None:
        return redirect("shopkeeper_dashboard")
    ...
    order = get_object_or_404(
        Order.objects
        .select_related("shop", "user", "address")
        .prefetch_related("items"),
        order_number=order_number,
        shop=profile.shop,
    )
    ...
    task = (
        OrderPickingTask.objects
        .filter(order=order, shop=profile.shop)
        .first()
    )

    if task is None or task.status not in {"PACKED", "HANDED_OVER"}:
        messages.error(
            request,
            "Complete exact product picking before opening rider details.",
        )
        return redirect("shopkeeper_pick_order", order_number=order_number)
    ...
    if assignment is None:
        try:
            assignment = _assign_available_rider(order)
        except Exception:
            assignment = None
    ...
    if assignment is not None:
        try:
            rider_profile = assignment.delivery_partner.delivery_profile
        except Exception:
            rider_profile = None

    return render(
        request,
        "customer/shopkeeper_rider_detail.html",
        {
            "profile": profile,
            "shop": profile.shop,
            "order": order,
...
def shopkeeper_order_action_view(request, order_number):
    ...
    with transaction.atomic():
        ...
        if action == "accept" and current_status == "pending":
            ...
            if (
                ...
                messages.error(
                    request,
                    "Picker has already started this order, "
            ...
            Settlement.objects.filter(order=order).update(
                status="On Hold"
            )

            messages.success(
                request,
                f"Order #{order.order_number} rejected successfully.",
            )

        else:
            messages.error(
                request,
                "This order action is not allowed now.",
            )
            return redirect("shopkeeper_orders")

    # Optional syncs are intentionally outside the main action transaction.
    # A secondary sync issue must not turn a successful Accept/Reject into 500.
    for product_id in product_ids_to_sync:
        try:
            product = Product.objects.filter(pk=product_id).first()
            if product:
    ...
    if accepted_now:
        try:
            ...
            messages.warning(
                request,
                "Order accepted. Picker queue sync is pending.",
            )
...
def delivery_assignment_action_view(request, assignment_id):
    ...
    with transaction.atomic():
        ...
        if action == "accept":
            ...
            messages.success(
                request,
                "Order accepted. Navigate to the pickup shop first."
            )
            return redirect(
...
@login_required
def razorpay_start_view(request):
    pending = request.session.get("amexa_pending_payment")
    ...
    address = get_object_or_404(
        Address,
        pk=pending.get("address_id"),
        user=request.user,
    )

    totals = _totals_with_coupon(request, items)
    ...
    try:
        requested_coins = int(pending.get("use_coins", "0") or 0)
    except (TypeError, ValueError):
        requested_coins = 0

    requested_coins = max(0, requested_coins)
    ...
    payable_total = max(
        Decimal("0.00"),
        totals["payable_total"] - coin_discount,
    )
    ...
    amount_paise = int(
        (payable_total * Decimal("100")).quantize(Decimal("1"))
    )
    ...
    request.session["amexa_razorpay_order_id"] = gateway_order["id"]
    request.session["amexa_razorpay_amount"] = amount_paise
    request.session.modified = True

    return render(
        request,
        "customer/razorpay_payment.html",
        {
...
def razorpay_verify_view(request):
    ...
    pending = request.session.get("amexa_pending_payment")
    server_order_id = request.session.get("amexa_razorpay_order_id")
    expected_amount = request.session.get("amexa_razorpay_amount")

    if not pending or not server_order_id or expected_amount is None:
        messages.error(request, "Payment session expired.")
        return redirect("checkout")

    payment_id = (
        request.POST.get("razorpay_payment_id", "") or ""
    ).strip()

    returned_order_id = (
        request.POST.get("razorpay_order_id", "") or ""
    ).strip()

    signature = (
        request.POST.get("razorpay_signature", "") or ""
    ).strip()
    ...
    if (
        ...
        or not signature
        or returned_order_id != server_order_id
    ):
    ...
    # Confirm gateway-side amount/order/status too.
    try:
        gateway_payment = client.payment.fetch(payment_id)

        if gateway_payment.get("order_id") != server_order_id:
            raise ValueError("Gateway order mismatch.")

        if int(gateway_payment.get("amount", 0)) != int(expected_amount):
            raise ValueError("Gateway amount mismatch.")

        status = gateway_payment.get("status")

        # Ensure money is captured before fulfilling the AMEXA order.
        if status == "authorized":
            try:
                gateway_payment = client.payment.capture(
                    payment_id,
                    int(expected_amount),
                )
            except Exception:
        ...
        if status != "captured":
            messages.error(
                request,
                "Payment is not captured yet. Order was not created."
            )
            return redirect("checkout")
    ...
    if not items:
        messages.error(
            request,
            "Payment succeeded but cart data is unavailable. "
    ...
    current_amount_paise = int(
        (current_total * Decimal("100")).quantize(Decimal("1"))
    )
    ...
    try:
        master_order, shop_orders = _create_multi_shop_order_from_cart(
            request.user,
            address,
            cart,
        ...
        # Payment is already captured. Try immediate full refund
        # if order creation cannot be completed.
        try:
            ...
            messages.error(
                request,
                f"{error} Payment refund has been initiated."
            )
        except Exception:
            messages.error(
                request,
                f"{error} Payment ID: {payment_id}. "
                "Please contact support."
            )

        return redirect("cart")

    payment = master_order.payment
    payment.payment_status = "Paid"
